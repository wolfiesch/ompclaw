...
import type { PairingRequestState, StoredPairingRequest, GatewayStore } from "./gateway-store";
...
function hashCode(salt: string, code: string): string {
  return createHash("sha256").update(Buffer.from(salt, "hex")).update(code, "utf8").digest("hex");
}
...
export class GatewayPairingStore {
  ...
  approve(
    ...
    const principalId =
      typeof principalIdOrResolver === "string" ? principalIdOrResolver : principalIdOrResolver(request.identity);
  ...
  #resolveCode(code: unknown, now: number): StoredPairingRequest {
    ...
    if (checkedCode === undefined) {
      this.#store.recordPairingFailures(
        pending.map((request) => request.identity),
        now,
      );
      throw new Error("pairing code is invalid or unavailable");
    }
    ...
    if (matches.length !== 1) {
      this.#store.recordPairingFailures(
        (matches.length === 0 ? pending : matches).map((request) => request.identity),
        now,
      );
      throw new Error("pairing code is invalid or unavailable");
    }
...
export class GatewayPairingService {
  ...
  request(identity: TransportIdentity, address: ConversationAddress, now = Date.now()): PairingRequestResult {
    ...
    return {
      request: this.#store.create(
        identity,
        address,
        code,
        this.#secrets.createSalt(),
        this.#maxAttempts,
        now,
        this.#ttlMs,
      ),
