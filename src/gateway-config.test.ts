...
describe("gateway config", () => {
  ...
  for (const args of [["--approval-mode", "always-ask"], ["--approval-mode=always-ask"]]) {
  ...
  test("defaults busy input mode to steer and forwards an explicit override to RPC", () => {
    ...
    expect(() => parseGatewayConfig({ omp: { busyInputMode: "queue" } })).toThrow(
      'omp.busyInputMode must be "steer" or "followup"',
    );
  });

  test("rejects invalid autonomy modes with the config path", () => {
    expect(() => parseGatewayConfig({ omp: { autonomyMode: "unattended" } })).toThrow(
      'omp.autonomyMode must be "inherit", "autopilot", "balanced", or "review"',
    );
  });
  ...
  for (const autonomyMode of ["autopilot", "balanced", "review"] as const) {
    for (const args of [["--approval-mode", "always-ask"], ["--approval-mode=always-ask"]]) {
      test(`rejects ${autonomyMode} autonomy with raw ${args[0]}`, () => {
        expect(() => parseGatewayConfig({ omp: { autonomyMode, args } })).toThrow(
          "omp.autonomyMode conflicts with --approval-mode in omp.args",
        );
      });
    }
  }
  ...
  test("keeps transactional updates opt-in with a fixed repository", () => {
    ...
    expect(() => parseGatewayConfig({ updates: { enabled: true, repository: "/repo", unknown: true } })).toThrow(
      "unknown key unknown",
    );
  });
  ...
  test("keeps experimental learning opt-in with an explicit memory model", () => {
    ...
    expect(() => parseGatewayConfig({ learning: { enabled: true, memoryModel: "unknown" } })).toThrow(
      "supported OMP memory model",
    );
  });
  ...
  test("keeps forum topic sessions opt-in and rejects implicit root topic creation", () => {
    const telegram = (topicSessions?: unknown) =>
      parseGatewayConfig({
        transports: {
          telegram: {
            enabled: true,
  ...
  test("accepts stdout and Whisper-style Telegram transcription commands", () => {
    const telegram = (transcribeCommand: unknown) =>
      parseGatewayConfig({
        transports: {
          telegram: {
            enabled: true,
