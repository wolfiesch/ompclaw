...
- **Package:** [`ompclaw`](https://www.npmjs.com/package/ompclaw) `0.9.1`
...
Branch protection requires the stable `verify` check. An automatic review is requested after every push. Before merging, confirm that the latest automated review commit matches the current pull-request head and that all review threads are resolved.
